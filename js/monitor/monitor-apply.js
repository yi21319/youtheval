...
function loadMonitorRecords() {
    ...
    const myRecords = monitorRecords.filter(r => r.monitorId === currentUser?.id);
...
function renderMonitorRecordsList(records) {
    ...
    records.forEach(record => {
        ...
        `;
        recordDiv.addEventListener('click', () => showMonitorRecordDetail(record));
        listContainer.appendChild(recordDiv);
    });
}

function showMonitorRecordDetail(record) {
...
function filterMonitorRecords(filter) {
...
function initMonitorRecordModal() {
...
function resetmonitorRecordForm() {
